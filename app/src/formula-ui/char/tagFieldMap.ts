...
Object.values(charSheets).forEach((sheet) =>
  Object.values(sheet).forEach((section) =>
    section.documents.forEach((doc) => {
      // Index fields from both `fields` docs and `conditional` docs so that
      // tags referenced only inside conditionals (e.g. opt target formulas)
      // still resolve to their localized field titles.
      const fields =
        doc.type === 'fields'
          ? doc.fields
