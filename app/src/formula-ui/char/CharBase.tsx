...
charBaseUiSheet.push(
  ...
  // lumiflux doesn't deal Anomaly DMG - exclude
  ...
  // wind and lumiflux have no disorder formula registered - exclude for now
  ...[
    ...allAttributeAnomalyKeys.filter((k) => k !== 'wind'),
    'frost' as const,
  ].map((attr): TagField => {
  ...
  // lumiflux doesn't accumulate Anomaly Buildup - exclude
  ...allAttributeAnomalyKeys.map(
    (attr): TagField => ({
      fieldRef: {
        et: 'own',
