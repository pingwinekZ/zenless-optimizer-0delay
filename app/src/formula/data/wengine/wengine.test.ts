import { prettify } from '@zenless-optimizer/common/util'
import {
  compileTagMapValues,
  setDebugMode,
...
describe('Disc sheets test', () => {
  ...
  it('DeepSeaVisitor', () => {
    ...
    expect(calc.compute(char.combat.crit_).val).toBeCloseTo(0.4)
  })
  ...
  it('HalfSugarBunny multi-member non-stacking', () => {
    const members = ['Billy', 'Ben', 'Seth']
    const data: TagMapNodeEntries = [
      ...teamData(members),
      ...withMember(
        'Billy',
    ...
    // Billy (no wengine) should get the team's non-stacked HalfSugarBunny buff
    const calcBilly = new Calculator(
      keys,
      values,
      compileTagMapValues(keys, data)
    ).withTag({ src: 'Billy', dst: 'Billy' })
    const billy = convert(ownTag, { et: 'own', src: 'Billy' })
    const billyHp = calcBilly.compute(billy.combat.hp_).val

    // Ben (w/ wengine) gets his own stackIn + team buff (same value, non-stacked)
    const calcBen = new Calculator(
      keys,
      values,
      compileTagMapValues(keys, data)
    ).withTag({ src: 'Ben', dst: 'Ben' })
    const ben = convert(ownTag, { et: 'own', src: 'Ben' })
    const benHp = calcBen.compute(ben.combat.hp_).val

    // Seth (w/ wengine) same as Ben
    const calcSeth = new Calculator(
      keys,
      values,
      compileTagMapValues(keys, data)
    ).withTag({ src: 'Seth', dst: 'Seth' })
    const seth = convert(ownTag, { et: 'own', src: 'Seth' })
    const sethHp = calcSeth.compute(seth.combat.hp_).val

    // All three should get the team buff 0.16 (non-stacked, applied once via team routing)
    expect(billyHp).toBeCloseTo(0.16, 2)
    expect(benHp).toBeCloseTo(0.16, 2)
    expect(sethHp).toBeCloseTo(0.16, 2)
  })

  it('HalfSugarBunny buff goes away when wengine is removed', () => {
    const members = ['Billy', 'Seth']
    // Billy has no wengine, Seth has HalfSugarBunny
    const dataWithBuff: TagMapNodeEntries = [
      ...teamData(members),
      ...withMember(
        'Billy',
        ...charTagMapNodeEntries({
    ...
    // Seth has the wengine → both get the buff
    const calcWithBuff = new Calculator(
      keys,
      values,
      compileTagMapValues(keys, dataWithBuff)
    ).withTag({ src: 'Billy', dst: 'Billy' })
    const billy = convert(ownTag, { et: 'own', src: 'Billy' })
    expect(calcWithBuff.compute(billy.combat.hp_).val).toBeCloseTo(0.16, 2)

    // Now remove HalfSugarBunny from Seth (no wengine entries)
    const dataWithoutBuff: TagMapNodeEntries = [
      ...teamData(members),
      ...withMember(
        'Billy',
        ...charTagMapNodeEntries({
      ...
      ...withMember(
        ...
        // No wengine → wengineTagMapNodeEntries is omitted
      ),
      own.common.critMode.add('avg'),
    ]

    const calcWithoutBuff = new Calculator(
      keys,
      values,
      compileTagMapValues(keys, dataWithoutBuff)
    ).withTag({ src: 'Billy', dst: 'Billy' })
    // Buff should be 0: no one has HalfSugarBunny equipped
    expect(calcWithoutBuff.compute(billy.combat.hp_).val).toBeCloseTo(0, 2)
  })
})
