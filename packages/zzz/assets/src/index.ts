...
import monsters from './gen/monsters'
...
/** Always returns the big w-engine image. */
export function wengineAsset(wk: WengineKey): string {
  return (wengines[wk]?.big ?? '') as string
}

/** Returns the boss monster image, or '' if it was not downloaded. */
export function monsterAsset(name: string): string {
  return ((monsters as Record<string, string>)[name] ?? '') as string
}
